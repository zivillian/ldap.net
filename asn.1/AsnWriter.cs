...
// and copies, so it's a #define opt-in.
        ...
        public void WriteEncodedValue(ReadOnlyMemory<byte> preEncodedValue)
        {
            ...
            if (reader.HasData)
            {
                throw new ArgumentException("The input to WriteEncodedValue must represent a single encoded value with no trailing data.", nameof(preEncodedValue));
            }
        ...
        private void WriteNamedBitList(Asn1Tag tag, Type tEnum, object enumValue)
        {
            ...
            {
                throw new ArgumentException("Named bit list operations require an enum with the [Flags] attribute.", nameof(tEnum));
            }
        ...
        private void PopTag(Asn1Tag tag, bool sortContents=false)
        {
            ...
            {
                throw new ArgumentException("Cannot pop the requested tag as it is not currently in progress.", nameof(tag));
            }
