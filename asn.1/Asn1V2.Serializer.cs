...
namespace System.Security.Cryptography.Asn1
{
    ...
    internal class AsnAmbiguousFieldTypeException : AsnSerializationConstraintException
    {
        public AsnAmbiguousFieldTypeException(FieldInfo fieldInfo, Type ambiguousType)
            : base($"Field '{fieldInfo.Name}' of type '{fieldInfo.DeclaringType.FullName}' has ambiguous type '{ambiguousType.Namespace}', an attribute derived from AsnTypeAttribute is required.")
        {
        }
    }
    ...
    public static class AsnSerializer
    {
            ...
            if (attr.AllowNull)
            {
                if (!CanBeNull(typeT))
                {
                    throw new AsnSerializationConstraintException(
                        String.Format("[Choice].AllowNull=true is not valid because type '{0}' cannot have a null value.", typeT.FullName));
                }
            }
            ...
            out Asn1Tag? explicitTag)
        {
            ...
            {
                throw new AsnSerializationConstraintException(
                    String.Format("Type '{0}' cannot be serialized or deserialized because it is not sealed or has unbound generic parameters.", typeT.FullName));
            }
                ...
                if (fieldData.TagType == UniversalTagNumber.ObjectIdentifier)
                {
                    return (value, writer) =>
                        writer.WriteObjectIdentifier(
                            tag,
                            (string)value ?? throw new CryptographicException("The OID value was invalid."));
                }
            ...
            if (typeT.IsArray)
            {
                ...
                {
                    throw new AsnSerializationConstraintException(
                        String.Format("Type '{0}' cannot be serialized or deserialized because it is a multi-dimensional array.", typeT.FullName));
                }
                ...
                {
                    throw new AsnSerializationConstraintException(
                        String.Format("Type '{0}' cannot be serialized or deserialized because it is an array of arrays.", typeT.FullName));
                }
            ...
            throw new AsnSerializationConstraintException(
                String.Format("Could not determine how to serialize or deserialize type '{0}'.", typeT.FullName));
        }
